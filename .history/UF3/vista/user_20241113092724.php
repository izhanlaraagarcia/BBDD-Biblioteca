...
// Cargar los libros desde biblioteca.xml y verificar si se cargó correctamente
$libros = simplexml_load_file('../assets/libreria.xml');
if ($libros === false) {
    echo "<p>Error al cargar el archivo de libros. Verifica que la ruta sea correcta.</p>";
    exit();
}
?>
<head>
...
<div>
    ...
    foreach ($libros->libro as $libro) {
        ...
        // Enlace al formulario de pedido, pasando título e ISBN
        echo "<a href='../modelo/formularioPedido.php?titulo=" . urlencode($titulo) . "&isbn=" . urlencode($isbn) . "'>Comprar</a>";
